...
2) Prevent duplicate notifications in short window.
3) Recapture always available (no permanent lock).
*/

const STORE_KEY = "sfsy_cookie";
const STORE_LAST_VALUE_KEY = "sfsy_cookie_last_value";
const STORE_LAST_TS_KEY = "sfsy_cookie_last_ts";
...
try {
  ...
  const lastValue = $persistentStore.read(STORE_LAST_VALUE_KEY) || "";
  const lastTs = parseInt($persistentStore.read(STORE_LAST_TS_KEY) || "0", 10);
  const sameValue = sfsyUrl === lastValue;
  const inCooldown = nowTs - lastTs < NOTIFY_COOLDOWN_SEC;

  if (sameValue && inCooldown) {
    return done({ sfsy_capture: false, dedup: true, sfsyUrl });
  }

  const copyText = `sfsyUrl\n\n${sfsyUrl}\n\nLong-press to copy.`;
  const copyPage = `data:text/plain;charset=utf-8,${encodeURIComponent(copyText)}`;

  $persistentStore.write(sfsyUrl, STORE_LAST_VALUE_KEY);
  $persistentStore.write(String(nowTs), STORE_LAST_TS_KEY);

  notifyWithOpenUrl(
    "SFSY Capture OK",
    "Tap notification to copy sfsyUrl",
    "Same value is muted for 120 seconds",
    copyPage
  );
